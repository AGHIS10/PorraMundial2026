"""Actualiza partidos.json con los cuartos de final reales.

Carga equipos desde CSV en input/ (sufijo - 4) o, si no hay CSV, resuelve
W89–W96 desde resultados.json. Sincroniza fecha/hora en CEST (España) con
...
INICIO_INDICE = 96  # partido id 97
FIN_INDICE = 100  # exclusivo, partido id 100
STAGE_CUARTOS = "QUARTER_FINALS"
...
CRUCE_CUARTOS: list[tuple[str, str]] = [
    ("W89", "W90"),  # id 97
    ("W93", "W94"),  # id 98
    ("W91", "W92"),  # id 99
    ("W95", "W96"),  # id 100
...
def ganadores_octavos(
    partidos: list[dict[str, Any]],
    resultados: list[Any],
) -> dict[str, str]:
    """Mapa W{id} → nombre del equipo clasificado en octavos."""
    ...
    if pendientes:
        raise ValueError(
            "Faltan resultados de octavos para resolver cuartos:\n  "
            + "\n  ".join(pendientes)
        )
    return ganadores


def resolver_cuartos_desde_ganadores(
    ganadores: dict[str, str],
) -> list[dict[str, str]]:
    """Genera local/visitante para los 4 cuartos a partir del cuadro FIFA."""
    partidos: list[dict[str, str]] = []
    for local_ph, visitante_ph in CRUCE_CUARTOS:
...
def buscar_csv_cuartos(directorio: Path) -> Path | None:
    candidatos = sorted(directorio.glob("*- 4.csv")) + sorted(directorio.glob("*-4.csv"))
...
def _emparejar_api(
    ...
    """Busca el partido en la API (orientación directa o invertida)."""
    candidatos = [
        partido_api
...
def sincronizar_horarios_api(
    ...
    """Actualiza fecha/hora CEST de cuartos emparejando equipos con la API."""
    quarter = [
        partido_api
        for partido_api in partidos_api
        if partido_api.get("stage") == STAGE_CUARTOS
    ]
    sin_emparejar: list[str] = []

    for indice in range(INICIO_INDICE, FIN_INDICE):
        partido = partidos[indice]
        emparejado = _emparejar_api(partido, quarter, alias)
...
def actualizar_partidos(
    partidos_cuartos: list[dict[str, str]],
...
def main() -> int:
    ...
    try:
        ...
        csv_path = (
            Path(sys.argv[1])
            if len(sys.argv) > 1
            else buscar_csv_cuartos(INPUT_DIR)
        )

        if csv_path is not None:
            partidos_cuartos = leer_partidos_csv(csv_path)
            origen = csv_path.name
        else:
            ganadores = ganadores_octavos(partidos, resultados)
            partidos_cuartos = resolver_cuartos_desde_ganadores(ganadores)
            origen = "ganadores de octavos"

        if api_key:
            partidos_api = consultar_api(api_key)

        actualizados = actualizar_partidos(partidos_cuartos, partidos_api, alias)
    except (FileNotFoundError, ValueError, json.JSONDecodeError, requests.RequestException) as exc:
        print(f"Error: {exc}", file=sys.stderr)
        return 1

    print(f"✓ {len(actualizados)} cuartos cargados desde {origen}\n")
